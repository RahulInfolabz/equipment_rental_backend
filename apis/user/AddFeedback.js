...
async function AddFeedback(req, res) {
  try {
    ...
    if (!booking_id || !service_id || !rating || !feedback) {
      return res.status(400).json({ success: false, message: "Booking ID, equipment ID, rating and feedback are required" });
    }
    ...
    await db.collection("feedbacks").insertOne({
      ...
      service_id: new ObjectId(service_id), // equip_id as per DD field name
